...
# Validate inputs
if [[ ! "${FORMULA_NAME}" =~ ^[a-zA-Z0-9_-]+$ ]]
then
  echo "Error: Invalid formula name '${FORMULA_NAME}'." >&2
  exit 1
fi

if [[ ! "${NEW_TAG}" =~ ^v?[0-9]+\.[0-9]+\.[0-9]+([._-].*)?$ ]]
then
...
# Verify the update
if ! grep -q "tag:      \"${NEW_TAG}\"" "${FORMULA_FILE}"
then
  echo "Error: Failed to update tag in '${FORMULA_FILE}'." >&2
  exit 1
fi

if ! grep -q "revision: \"${NEW_SHA}\"" "${FORMULA_FILE}"
then
